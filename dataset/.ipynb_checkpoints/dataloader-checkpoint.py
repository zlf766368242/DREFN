...
class H36motion3D(Dataset):
    def __init__(self, actions='all', input_n=10, output_n=10, split=0, scale=100, sample_rate=2):
        ...
        """
        path_to_data = '/root/dataDisk/MAGCN/data/h3.6m/dataset'
        ...
        acts = define_actions(actions)
        subjs = subs[split]
        all_seqs, dim_ignore, dim_used = load_data_3d(path_to_data, subjs, acts, sample_rate, input_n + output_n)
    ...
    def __len__(self):
        return np.shape(self.all_seqs)[0] #B=?
...
class CMU_Motion3D(Dataset):
    def __init__(self, actions, input_n=10, output_n=10, split=0, dim_used=0, scale=100):
        ...
        all_seqs, dim_ignore, dim_use = load_data_cmu_3d(path_to_data, actions,
                                                                    input_n, output_n,
                                                                    is_test=is_test)
        if not is_test:
            dim_used = dim_use
...
class Pose3dPW3D(Dataset):
    def __init__(self, input_n=10, output_n=30, dct_n=15, split=0, scale=100, debug=False):
        path_to_data = '/root/lanyun-tmp/MAGCN/data/3dpw/sequenceFiles'
        self.path_to_data = path_to_data
        self.split = split
        self.dct_n = dct_n
        self.input_n = input_n
        ...
        new_order = [0, 1, 4, 7, 10, 2, 5, 8, 11, 3, 6, 9, 12, 15, 13, 16, 18, 20, 22, 14, 17, 19, 21, 23]
        new_order = np.array(new_order) * 3
        final_order = []
        for i in new_order:
            final_order.extend([i, i + 1, i + 2])
        all_seqs = all_seqs[:, :, final_order]
