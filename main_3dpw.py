...
def main():
    ...
    dataset_test = Pose3dPW3D(input_n=args.past_length, output_n=args.future_length, split=2, scale=args.scale)
    loaders_test = torch.utils.data.DataLoader(dataset_test, batch_size=128, shuffle=False, drop_last=False,
                                                num_workers=8)
    ...
    for epoch in range(0, args.epochs):
        ...
        if epoch % args.test_interval == 0:
            mpjpe = test(model, optimizer, epoch, ('all', loaders_test), dim_used, backprop=False)
            avg_mpjpe = mpjpe
            ...
            if avg_avg_mpjpe < best_test_loss:
                ...
                if args.model_save_name == 'default':
                    if args.future_length == 30 or args.future_length == 10:
                        file_path = os.path.join(args.model_save_dir, '3dpw_ckpt_long_best.pth')
                    else:
                        file_path = os.path.join(args.model_save_dir, '3dpw_ckpt_best.pth')
                else:
                    if args.future_length == 30 or args.future_length == 10:
                        file_path = os.path.join(args.model_save_dir, args.model_save_name+'3dpw_ckpt_long_best.pth')
                    else:
                        file_path = os.path.join(args.model_save_dir, args.model_save_name+'3dpw_ckpt_best.pth')
            ...
            if args.model_save_name == 'default':
                if args.future_length == 30 or args.future_length == 10:
                    file_path = os.path.join(args.model_save_dir, '3dpw_ckpt_long_'+str(epoch)+str(avg_avg_mpjpe)+'.pth')
                else:
                    file_path = os.path.join(args.model_save_dir, '3dpw_ckpt_'+str(epoch)+str(avg_avg_mpjpe)+'.pth')
            else:
                if args.future_length == 30 or args.future_length == 10:
                    file_path = os.path.join(args.model_save_dir, args.model_save_name+'3dpw_ckpt_long_'+str(epoch)+str(avg_avg_mpjpe)+'.pth')
                else:
                    file_path = os.path.join(args.model_save_dir, args.model_save_name+'3dpw_ckpt_'+str(epoch)+str(avg_avg_mpjpe)+'.pth')
            torch.save(state, file_path)
