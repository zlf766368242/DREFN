...
from model.MAGCNmodelGRU_cmu_short import MultiScaleModel
from model.MAGCNmodelGRU_cmu_long import MultiScaleModel2
...
def main():
    ...
    acts = ["basketball", "basketball_signal", "directing_traffic", "jumping", "running", "soccer", "walking","washwindow"]

    loaders_test = {}
    for act in acts:
        dataset_test = CMU_Motion3D(actions=act, input_n=args.past_length, output_n=args.future_length, split=1, scale=args.scale,dim_used = dataset_train.dim_used)
        ...
        if epoch % args.test_interval == 0:
            ...
            if avg_avg_mpjpe < best_test_loss:
                ...
                if args.model_save_name == 'default':
                    if args.future_length == 25 or args.future_length == 13:
                        file_path = os.path.join(args.model_save_dir, 'cmu_ckpt_long_best.pth')
                    else:
                        file_path = os.path.join(args.model_save_dir, 'cmu_ckpt_best.pth')
                else:
                    if args.future_length == 25 or args.future_length == 13:
                        file_path = os.path.join(args.model_save_dir, args.model_save_name+'_cmu_ckpt_long_best.pth')
                    else:
                        file_path = os.path.join(args.model_save_dir, args.model_save_name+'_cmu_ckpt_best.pth')
            ...
            if args.model_save_name == 'default':
                if args.future_length == 25 or args.future_length == 13:
                    file_path = os.path.join(args.model_save_dir, 'cmu_ckpt_long_'+str(epoch)+str(avg_avg_mpjpe)+'.pth')
                else:
                    file_path = os.path.join(args.model_save_dir, 'cmu_ckpt_'+str(epoch)+str(avg_avg_mpjpe)+'.pth')
            else:
                if args.future_length == 25 or args.future_length == 13:
                    file_path = os.path.join(args.model_save_dir, args.model_save_name+'_cmu_ckpt_long_'+str(epoch)+str(avg_avg_mpjpe)+'.pth')
                else:
                    file_path = os.path.join(args.model_save_dir, args.model_save_name+'_cmu_ckpt_'+str(epoch)+str(avg_avg_mpjpe)+'.pth')
            torch.save(state, file_path)
...
def test(model, optimizer, epoch, act_loader,dim_used=[],backprop=False):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(loader):
            ...
            loc_pred = model.predict(all_traj)
