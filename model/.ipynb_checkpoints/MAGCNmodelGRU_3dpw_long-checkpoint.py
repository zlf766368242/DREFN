...
class St_gcn(nn.Module):
    ...
    def forward(self, x, A_skl):
        ...
        x = self.tcn(x) + res
        return self.relu(x)
class activatelayerj(nn.Module):
...
class activatelayerb(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, t_kernel_size=1, stride=1, dropout=0.,
                 residual=True):
        super().__init__()
        assert len(kernel_size) == 2
...
class STblockj(nn.Module):
    def __init__(self, 
        ...
        if mlp_head:
            self.head = nn.Sequential(nn.Linear(decoder_dim,32),nn.Linear(32,3))
        else:
            self.head = nn.Linear(decoder_dim, 3*self.pred_time)
        if mlp_head:
            self.aux_head = nn.Sequential(nn.Linear(decoder_dim,32),nn.Linear(32,3))
            self.aux_head_2 = nn.Sequential(nn.Linear(decoder_dim,32),nn.Linear(32,3))
        else:
        ...
        self.activite = activatelayerj(in_dim, 32, kernel_size)
        self.st_gcn1 = St_gcn(decoder_dim, 96, kernel_size, 1, 1)
    ...
    #111111111111111111111111111111111111111111111111111111111111111111111111
        ...
        pred_mask_coord_values_noise = self.aux_head(dec_mask_tokens_noise)
    ...
    def __init__(self, 
        ...
        if self.multi_output:
            ...
            for i in range(self.encoder_depth):
                if mlp_head:
                    self.head.append(MLP(decoder_dim, 3,hidden_size=(128,)))
                else:
                    self.head.append(nn.Linear(decoder_dim, 3))
            self.head = nn.ModuleList(self.head)
        else:
            if mlp_head:
                self.head = MLP(decoder_dim, 3,hidden_size=(decoder_dim,))
            else:
                self.head = nn.Linear(decoder_dim, 3)
        self.same_head = same_head
        if not same_head:
            if mlp_head:
                self.aux_head = MLP(decoder_dim, 3,hidden_size=(decoder_dim,))
                self.aux_head_2 = MLP(decoder_dim, 3,hidden_size=(decoder_dim,))
            else:
        ...
        self.encoder = nn.ModuleList(self.encoder)
        self.decoder = nn.ModuleList(self.decoder)
        self.activite = activatelayerb(in_dim, 32, kernel_size)
        self.st_gcn1 = St_gcn(decoder_dim, 96, kernel_size, 1, 1)
        self.scale3 = AveargePart()
        self.s3_nback = nn.Linear(5,23)
        self.s3_back = nn.Linear(5,23)
    def forward(self,all_traj):
        start = all_traj[:, :, self.past_timestep - 1:self.past_timestep]
...
class AveargePart(nn.Module):
    ...
    def __init__(self):
        ...
        self.left_arm = [13, 14, 15, 16, 17]
        self.right_arm = [18, 19, 20, 21, 22]
    ...
    def forward(self, x):
        ...
        x_leftarm = F.avg_pool2d(x[:, :, :, self.left_arm], kernel_size=(1, 5))  # [N, C, T, V=1]
        x_rightarm = F.avg_pool2d(x[:, :, :, self.right_arm], kernel_size=(1, 5))  # [N, C, T, V=1]
        x_body = torch.cat((x_leftleg, x_rightleg, x_torso, x_leftarm, x_rightarm),
                           dim=-1)  # [N, C, T, V=1]), dim=-1)        # [N, C, T, 5]
        return x_body
        
class MultiScaleModel(nn.Module):
    def __init__(self, in_dim, h_dim, past_timestep, future_timestep, kernel_size, graph_args_j,
                 graph_args_b):
        ...
        self.st_block_b = STblockb(
            in_dim=in_dim, h_dim=h_dim, past_timestep=past_timestep, future_timestep=future_timestep,
            kernel_size=kernel_size,
            graph_args_b=graph_args_b)

        #self.mlp = MLP(input_dim=64,output_dim=3,hidden_size=(32,))
        self.mlp1 =  nn.Linear(96,3)
        self.mlp2 =  nn.Linear(96,3)
        self.ffn = FFN(96, 128, dropout=0.)
        self.cross_attention_jb = nn.MultiheadAttention(embed_dim=96, num_heads=8, dropout=0.,batch_first=True)
    def forward(self, x):
